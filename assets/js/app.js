(function () {
  ...
  async function installSafeServiceWorker() {
    ...
    try {
      var reg = await navigator.serviceWorker.register('./sw.js?v=' + encodeURIComponent(cfg.version || '1.1.4'), { scope: './' });
  ...
  async function render() {
      ...
      '<section class="ws-app">' +
        ...
        '<div class="ws-grid">' +
          '<article class="ws-card"><h3>État technique</h3><ul class="ws-status-list">' +
            ...
            '<li><span>Version</span><span>' + escapeHtml(cfg.version || '1.1.4') + '</span></li>' +
          '</ul></article>' +
          ...
          card('Cache PWA maîtrisé', 'Le service worker V1.1.4 nettoie les anciens caches et privilégie le réseau pour éviter de servir une ancienne version cassée.') +
    ...
    boot.finish('WebStorming OS V1.1.4 prêt.');
  }
